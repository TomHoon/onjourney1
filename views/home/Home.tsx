...
import {
	Camera, Image as ImageIcon, Book, Car, Upload, Settings2,
	Truck, ArrowRight, Menu, X, ChevronLeft, Check, CreditCard,
	Smartphone, UploadCloud
} from 'lucide-react';
...
export default function Home() {
	...
	const products: Product[] = [
		...
		{ id: 'print', title: "프리미엄 사진인화", icon: <ImageIcon />, color: "bg-amber-100", price: 5000 }
	];
			...
			<div className="min-h-screen bg-white font-sans text-gray-800">
					...
					<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
						<div className="flex justify-between items-center h-16">
							...
							<div className="hidden md:flex items-center space-x-8">
								<a href="#products" className="text-gray-600 hover:text-blue-600 font-medium transition-colors">상품 소개</a>
								<button onClick={startOrder} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-full font-semibold transition-all shadow-md">
									주문하기
								</button>
							</div>
				...
				<section className="pt-32 pb-20 px-4 text-center max-w-7xl mx-auto">
					<h1 className="text-4xl md:text-6xl font-extrabold text-gray-900 mb-6 leading-tight">
						스마트폰 속 사진이<br/><span className="text-blue-600">특별한 굿즈</span>가 됩니다
					</h1>
					<p className="text-lg text-gray-600 mb-10 max-w-2xl mx-auto">
						차량용 디퓨저부터 다이어리까지, 당신의 소중한 순간을 일상의 아이템으로 소장하세요.
					</p>
					<button onClick={startOrder} className="bg-blue-600 text-white px-10 py-4 rounded-full font-bold text-xl shadow-xl hover:bg-blue-700 transition-all transform hover:-translate-y-1">
						지금 제작 시작하기
					</button>
				</section>
				...
				<section id="products" className="py-20 bg-gray-50">
					<div className="max-w-7xl mx-auto px-4 grid md:grid-cols-3 gap-8">
						{products.map(p => (
							<div key={p.id} className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-lg transition-shadow">
								{/* 배경색이 확실히 보이도록 opacity와 보색 대비를 고려한 클래스 적용 */}
								<div style={{color: 'black'}} className={`${p.color} w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-2xl text-gray-700 shadow-inner`}>
									{p.icon}
								</div>
								<h3 className="text-2xl font-bold mb-3">{p.title}</h3>
								<p className="text-gray-600 mb-6">고해상도 인쇄와 최고급 부자재로 제작되는 {p.title}를 만나보세요.</p>
								<button onClick={startOrder} className="w-full py-3 rounded-xl border border-gray-900 font-bold hover:bg-gray-900 hover:text-white transition-all">
									이 상품 선택하기
								</button>
							</div>
						))}
		...
		<div className="min-h-screen bg-gray-50 pt-20 pb-10">
			<div className="max-w-3xl mx-auto px-4">
				...
				<div className="flex items-center justify-between mb-8">
					<button
						onClick={() => orderStep === 1 ? setView('landing') : setOrderStep(prev => prev - 1)}
					...
					<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
						...
						<div className="grid gap-4">
							{products.map(p => (
								<button
									key={p.id}
									onClick={() => { setOrderData({...orderData, product: p}); setOrderStep(2); }}
								...
								>
									<div style={{color: 'black'}} className={`w-14 h-14 rounded-xl flex items-center justify-center mr-6 text-xl ${p.color}`}>{p.icon}</div>
									<div className="text-left">
										<h3 style={{color: 'black'}} className="font-bold text-lg">{p.title}</h3>
										<p className="text-sm text-blue-600 font-medium">{p.price.toLocaleString()}원</p>
									</div>
									<div style={{color: 'black'}} className="ml-auto"><ArrowRight className="text-gray-300" /></div>
								</button>
							))}
						</div>
					</div>
				...
				{orderStep === 2 && (
					<div className="animate-in fade-in slide-in-from-bottom-4">
						<h2 style={{color: 'black'}} className="text-3xl font-bold mb-2">사진 레이아웃</h2>
						<p className="text-gray-500 mb-8">4x6 비율 내에서 사진이 배치될 구성을 선택하세요.</p>
						<div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
							{templates.map(t => (
								<button
									key={t.id}
									onClick={() => { setOrderData({...orderData, template: t}); setOrderStep(3); }}
								...
								>
									...
									<p style={{color: 'black'}} className="font-bold text-center">{t.title}</p>
								</button>
							))}
						</div>
					</div>
				...
				{orderStep === 3 && orderData.template && (
					<div className="animate-in fade-in slide-in-from-bottom-4 text-center">
						<h2 style={{color: 'black'}} className="text-3xl font-bold mb-2">이미지 채우기</h2>
						...
						<div className="max-w-xs mx-auto aspect-[4/6] bg-white rounded-2xl shadow-2xl p-4 border border-gray-100 flex gap-2">
							{Array.from({length: orderData.template.slots}).map((_, i) => (
								...
								>
									{orderData.images[i] ? (
										<img src={orderData.images[i]} className="w-full h-full object-cover" alt="preview" />
									) : (
				...
				{orderStep === 4 && (
					<div className="animate-in fade-in slide-in-from-bottom-4">
						<h2 style={{color: 'black'}} className="text-3xl font-bold mb-2">문구 추가</h2>
						...
						<div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
							...
							<div className="flex justify-between mt-4 px-1">
								...
								<span className={`text-xs font-bold ${orderData.caption.length === 20 ? 'text-red-500' : 'text-blue-500'}`}>
                  {orderData.caption.length}/20
                </span>
							</div>
						</div>
						
						<button onClick={() => setOrderStep(5)} className="w-full mt-8 bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg">
							최종 확인 및 결제
						</button>
					</div>
				)}
				...
				{orderStep === 5 && orderData.product && orderData.template && (
					<div className="animate-in fade-in slide-in-from-bottom-4 space-y-8">
						<h2 style={{color: 'black'}} className="text-3xl font-bold">최종 확인</h2>
							...
							<div className="w-24 aspect-[4/6] bg-gray-100 rounded-lg flex gap-1 p-1 border">
								{Array.from({length: orderData.template.slots}).map((_, i) => (
									<div key={i} className="flex-1 bg-gray-200 rounded-sm overflow-hidden">
										{orderData.images[i] && <img src={orderData.images[i]} className="w-full h-full object-cover" alt="summary" />}
									</div>
								))}
							</div>
							<div className="flex-1 flex flex-col justify-center">
								<p className="text-sm font-bold text-blue-600 uppercase tracking-tighter mb-1">Your Selection</p>
								<h3 style={{color: 'black'}} className="text-2xl font-black">{orderData.product.title}</h3>
								<p className="text-gray-500 text-sm mb-4">{orderData.template.title} 레이아웃</p>
								<p style={{color: 'black'}} className="text-xl font-bold">{orderData.product.price.toLocaleString()}원</p>
							</div>
						</div>
						...
						<div className="bg-white p-8 rounded-3xl shadow-md border border-gray-100 space-y-4">
							...
							<input
								...
								onChange={(e) => setOrderData({...orderData, userInfo: {...orderData.userInfo, name: e.target.value}})}
							/>
							<div className="flex gap-2">
								<input
									...
									onChange={(e) => setOrderData({...orderData, userInfo: {...orderData.userInfo, phone: e.target.value}})}
								/>
								<button onClick={handleAuthRequest} className="bg-gray-900 text-white px-6 rounded-xl font-bold text-sm">인증번호</button>
							</div>
							<div className="flex gap-2">
								<input
									type="text"
									...
									style={{color: 'black'}}
									className="flex-1 bg-gray-50 border-0 rounded-xl p-4"
									onChange={(e) => setOrderData({...orderData, userInfo: {...orderData.userInfo, authCode: e.target.value}})}
								/>
								<button onClick={handleAuthConfirm} className="bg-blue-600 text-white px-6 rounded-xl font-bold text-sm">확인</button>
							</div>
							{orderData.userInfo.isAuthed && (
								<div style={{color: 'black'}} className="text-center p-3 bg-green-50 text-green-700 rounded-xl text-sm font-bold border border-green-100">
									인증되었습니다. 결제를 진행해주세요.
								</div>
							)}
						</div>
				...
				{orderStep === 6 && (
					<div className="text-center py-12 animate-in zoom-in duration-500">
						...
						<p className="text-gray-500 mb-10 text-lg">
							입금 확인 후 바로 제작이 시작됩니다.
						</p>
						<button onClick={() => setView('landing')} className="bg-gray-900 text-white px-12 py-4 rounded-full font-bold">홈으로 가기</button>
					</div>
				)}
				...
				{orderStep === 7 && (
					<div className="text-center py-12 animate-in fade-in">
						...
						<button onClick={() => setOrderStep(5)} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold mb-4">다시 시도</button>
					</div>
				)}
			</div>
		</div>
